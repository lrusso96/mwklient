...
By default, mwklient will connect using https. If your site doesn't support
...
`$wgScriptPath`_. mwklient defaults to the script path '/w/' used by the
Wikimedia wikis. If you get a 404 Not Found or a
:class:`mwklient.errors.InvalidResponse` error upon connecting, your site might
...
Note that mwklient appends ' - mwklient/{version} ({url})' to your string.
...
:class:`mwklient.errors.MwklientError`.

Authenticating
--------------

mwklient supports several methods for authentication described below. By default
it will also protect you from editing when not authenticated by raising a
:class:`mwklient.errors.LoginError`. If you actually *do* want to edit
...
If login fails, a :class:`mwklient.errors.LoginError` will be thrown.
See :meth:`mwklient.client.Site.login` for all options.
