...
class Image(mwklient.page.Page):
    ...
    def imagehistory(self):
        ...
        return mwklient.listing.PageProperty(self, 'imageinfo', 'ii',
                                             iiprop=iiprop)
    ...
    def imageusage(self, namespace=None, filterredir='all', redirect=False,
        ...
        """
        prefix = mwklient.listing.List.get_prefix('iu', generator)
        kwargs = dict(mwklient.listing.List.generate_kwargs(
            prefix, title=self.name, namespace=namespace,
            filterredir=filterredir))
        if redirect:
            kwargs['%sredirect' % prefix] = '1'
        return mwklient.listing.List.get_list(generator)(
            self.site, 'imageusage', 'iu', limit=limit, return_values='title',
            **kwargs)
    ...
    def duplicatefiles(self, limit=None):
        ...
        """
        return mwklient.listing.PageProperty(self, 'duplicatefiles', 'df',
                                             dflimit=limit)

    def download(self, destination=None):
        """
        Download the file. If `destination` is given, the file will be written
        directly to the stream. Otherwise the file content will be stored in
        memory and returned (with the risk of running out of memory for large
        files).
    ...
    def __repr__(self):
        return "<Image object '%s' for %s>" % (self.name.encode('utf-8'),
                                               self.site)
